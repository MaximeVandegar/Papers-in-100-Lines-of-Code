"""
Adapted from:
https://github.com/MaximeVandegar/Papers-in-100-Lines-of-Code/tree/main/3D_Gaussian_Splatting_for_Real_Time_Radiance_Field_Rendering

Original source: Maxime Vandegar, Papers in 100 Lines of Code
Module: 3D Gaussian Splatting for Real-Time Radiance Field Rendering
"""
...
def evaluate_sh(f_dc, f_rest, points, c2w):
    ...
    view_dir = view_dir / view_dir.norm(dim=-1, keepdim=True).clamp_min(1e-6)
...
def camera_space_points(pc, c2w):
...
def project_points(x, y, z, fx, fy, cx, cy):
    uv = torch.stack([fx * x / z + cx, fy * y / z + cy], dim=-1)
    return uv


def inv2x2(M, eps=1e-6):
...
def render(pos, color, opacity_raw, sigma, c2w, H, W, fx, fy, cx, cy,
    ...
    final_image = torch.ones((H * W, 3), device=pos.device, dtype=pos.dtype)

    x, y, z = camera_space_points(pos, c2w)

    depth_mask = (z > near) & (z < far)
    pos = pos[depth_mask]
    color = color[depth_mask]
    opacity = torch.sigmoid(opacity_raw[depth_mask]).clamp(0, 0.999)
    sigma = sigma[depth_mask]
    x = x[depth_mask]
    y = y[depth_mask]
    z = z[depth_mask]
    idx = torch.nonzero(depth_mask, as_tuple=False).squeeze(1)

    uv = project_points(x, y, z, fx, fy, cx, cy)
    ...
    eye2 = torch.eye(2, device=sigma_camera.device, dtype=sigma_camera.dtype).unsqueeze(0)
    sigma_camera = sigma_camera + min_conis * eye2
    ...
    # Tiling — eigvalsh under no_grad: only used for culling radius, no gradients needed
    with torch.no_grad():
        major_variance = torch.linalg.eigvalsh(sigma_camera)[:, 1].clamp(1e-12, 1e4)
    ...
    if not on_screen.any():
        return final_image.reshape((H, W, 3))
    ...
    inverse_covariance = inv2x2(sigma_camera, eps=min_conis)
    ...
    for tile_id, s0, s1 in zip(unique_tile_ids.tolist(), start.tolist(), end.tolist()):
        ...
        q = torch.clamp(q, min=0.0, max=chi_square_clip)

        inside = q <= chi_square_clip
        g = torch.exp(-0.5 * q)  # [N, T * T]
        ...
        rgb = (w.unsqueeze(-1) * gaussian_i_color.unsqueeze(1)).sum(dim=0)   # [T*T, 3]
        alpha = w.sum(dim=0).clamp(0, 1).unsqueeze(-1)                       # [T*T, 1]
        final_image[pixel_idx_1d] = rgb + (1.0 - alpha)
