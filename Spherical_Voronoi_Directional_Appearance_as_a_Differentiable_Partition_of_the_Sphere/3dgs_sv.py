...
def evaluate_sv(sv_sites, sv_color, points, c2w):
    ...
    # Eq. 5 from https://arxiv.org/pdf/2512.14180
    scores = torch.einsum("nkd,nd->nk", sv_sites, view_dir)  # [N, K]
    weights = torch.softmax(scores, dim=1)  # [N, K]
    # Eq. 4 from https://arxiv.org/pdf/2512.14180
    logits = (weights.unsqueeze(-1) * sv_color).sum(dim=1)  # [N, 3]
...
if __name__ == "__main__":
    ...
    pos = torch.load('trained_gaussians_sv/bonsai/pos_30000.pt').cuda()
    opacity_raw = torch.load('trained_gaussians_sv/bonsai/opacity_raw_30000.pt').cuda()
    sv_sites = torch.load('trained_gaussians_sv/bonsai/sv_sites_30000.pt').cuda()
    sv_color = torch.load('trained_gaussians_sv/bonsai/sv_color_30000.pt').cuda()
    scale_raw = torch.load('trained_gaussians_sv/bonsai/scale_raw_30000.pt').cuda()
    q_raw = torch.load('trained_gaussians_sv/bonsai/q_rot_30000.pt').cuda()

    cam_parameters = np.load('out_colmap/bonsai/cam_meta.npy', allow_pickle=True).item()
    orbit_c2ws = torch.load('camera_trajectories/bonsai_orbit.pt').cuda()
    ...
    with torch.no_grad():
        for i, c2w_i in tqdm(enumerate(orbit_c2ws)):
            ...
            color = evaluate_sv(sv_sites, sv_color, pos, c2w)
